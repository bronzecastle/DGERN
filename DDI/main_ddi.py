...
def cross_validation(drug_drug_matrix, CV_num):
    ...
    for CV in range(0, CV_num):
        ...
        g = graph.Graph()
        g.read_edgelist('./data/drug_drug.txt')
        ...
        print("Test Begin")
        model = sdne.SDNE(g, [1000, 32],)
        print("Test End")

        data = pd.DataFrame(model.vectors).T
        data.to_csv('./data/embeddings1/d_embeddings.csv', header=None)

        I1 = []
        with open('./data/embeddings1/d_embeddings.csv', "rt", encoding='utf-8')as csvfile1:
            reader = csv.reader(csvfile1)
            for i in reader:
                I1.append(i[0])
        I1.sort()

        E=np.zeros((365, 32), float)
        for i in I1:
            E[int(i)-1][0:32] = model.vectors[str(i)]
...
def Interections():
    ...
    with open('./data/Nodes.csv', "rt", encoding='utf-8')as csvfile:
    ...
    with open("./data/drugs_vector1.csv", "rt", encoding="utf-8") as csvfile:
        reader = csv.reader(csvfile)
        for i in reader:
            Drug.append(i[0])

    with open('./data/Drug_Information.csv', "rt", encoding='utf-8')as csvfile:
        reader = csv.reader(csvfile)
        for i in reader:
            D.append(i[0])
            I.append(i[1])
