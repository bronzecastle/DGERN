...
class SAE(object):
    ...
    def __init__(self, 
        ...
        self.autoencoders, self.encoder, self.dnn= self.make_autoencoders()
        plot_model(self.autoencoders, show_shapes=True, 
                   to_file=os.path.join(save_dir,'autoencoders.png'))
    ...
    def make_autoencoders(self):
        ...
        # DNN output
        z = Dense(18, kernel_initializer=self.choose_init(init=self.init,seed=self.random_seed+18), activation='softmax')(h)
        return Model(inputs=x, outputs=y,name="AE"),Model(inputs=x,outputs=h,name="encoder"),Model(inputs=x,outputs=z,name="dnn")
    ...
    def pretrain_dnn(self, x, y, epochs=300):
        print('Contrust a new dnn model to make class mission, pretrain this model now.')
        simple_adam = keras.optimizers.Adam()
        callbacks=[EarlyStopping(monitor='loss',min_delta=1e-4,patience=50,verbose=1,mode='auto',restore_best_weights = True)]
        self.dnn.compile(loss='categorical_crossentropy', optimizer=simple_adam, metrics=['accuracy'])
        self.hist = self.dnn.fit(x, y, batch_size=self.batch_size, callbacks=callbacks, epochs=epochs, shuffle=True, verbose=1)

    def fit(self, x, y, epochs=300,decaying_step=3): # use stacked autoencoder pretrain and fine tuning
        self.pretrain_stacks(x, epochs=int(epochs/2),decaying_step=decaying_step)
        self.pretrain_autoencoders(x, epochs=epochs)
        self.pretrain_dnn(x, y, epochs=epochs*2)
